...
def get_subfolder_seq(dataset, split):
	# dataset: KITTI/nuScenes/aihub, split: val/test
	...
	if dataset == 'KITTI':				# KITTI
		...
		# train과 val은 'training' split의 일부만 사용하고 test는 'testing'폴더 안의 전부 
		...
		data_root = os.path.join('G:/내 드라이브/data/kittimot') 		# path containing the KITTI root 
		...
		split = split.lower()
		det_id2str = {1: 'vehicle/sedan', 2: 'vehicle/suv', 3:'vehicle/truck', 4:'vehicle/bus', 5:'vehicle/etc',
				6:'bicycle/auto_bicycle', 7:'bicycle', 8:'pedestrian', 9:'pedestrian/adult', 10:'pedestrian/child'
		}
		data_root = os.path.join('G:/내 드라이브/data', 'AIhub/Auto3D')
		if split.lower() == 'train' or split.lower() == 'val':
			subfolder = 'Training'
		...
		seq = os.listdir(os.path.join(data_root, subfolder, 'seoul1_raw'))
		seq.sort()
		seq_num = len(seq)

		if split == 'train': seq_eval = seq[:int(seq_num*0.8)]
		elif split == 'val': seq_eval = seq[int(seq_num*0.8):]
		elif split == 'test': seq_eval = seq
...
def initialize(cfg, data_root, save_dir, subfolder, seq_name, cat, ID_start, hw, log_file):
    # data_root: C:/Users/Admin/Documents/GitHub/AB3DMOT/data/KITTI 
    # subfolder: training, val, testing
    # seq_name: 0001, 0006, ...
    # cat: car, pedestrian, cyclist
    # wh
